...
#Species that we can have an overview on different thresholds for each sex

#For Trachemys scripta and Podocnemis vogli (separate female/male thresholds)

preds.separate <- list()

max.sizes.separate <- list()
max.sizes.separate$Podocnemis_vogli <- setNames(c(360,277),c('Female','Male'))
max.sizes.separate$Trachemys_scripta <- setNames(c(328,261),c('Female','Male'))


taxa <- c('Podocnemis_vogli','Trachemys_scripta')

for ( taxon in 1:length(taxa)){
  
  take = taxa[taxon]
  
  preds.separate[[taxon]] <- list()
  
  coords.tmp <- GPA$coords[,,names(groups.list[[take]])]
  
  size.tmp <- GPA$Csize[names(groups.list[[take]])]
  
  specimens <- sapply(strsplit(names(size.tmp),'_'), function(x) paste0(x[3],'_',x[4]) )
  sex <- subset(landmark_data,Name==take)
  sex.museum.number <- paste0(sex$Museum,'_',sex$Museum.Number)
  sex.museum.number <- !duplicated(sex.museum.number)
  sex <- sex$Sex[ sex.museum.number ]
  females <- which(sex!='M')
  males <- which(sex=='M')
  
  #Get females
  coords.tmp <- coords.tmp[,,females]
  
  proctmp <- procD.lm( coords.tmp ~ log10(size.tmp)[females]    )
  #summary(proctmp)
  ...
  max.GPA <- c(1,log10(max.sizes.separate[[take]]['Female'] )) %*% matrix(coef(linear.size))
  max.GPA <- c(max.GPA)
  sizes <- seq(min(proctmp$X[,2]) , max.GPA, length.out=1000)
  ...
  #Also use real maximum as the maximum in the sequence below
  min.real <- min(subset(landmark_data,Name==take)$Length[females])
  real.sizes <- seq(log10(min.real) , log10(max.sizes.separate[[take]]['Female']) ,
                    length.out=1000)
  ...
  preds.separate[[taxon]]$PredLine <- list()
  
  preds.separate[[taxon]]$PredLine$Females <- data.frame('Predicted_shape'=preds,'CS'=10^sizes, 
                                                         'SCL'=10^real.sizes)
  ######
  ...
  preds.separate[[taxon]]$CAC <- list()
  
  preds.separate[[taxon]]$CAC$Females <- data.frame('Predicted_shape'=preds,'CS'=10^sizes,
                                                    'SCL'=10^real.sizes)
  ######
  ...
  #Get Males
  coords.tmp <- GPA$coords[,,names(groups.list[[take]])]
  coords.tmp <- coords.tmp[,,males]
  
  proctmp <- procD.lm( coords.tmp ~ log10(size.tmp)[males]    )
  #summary(proctmp)
  ...
  max.GPA <- c(1,log10(max.sizes.separate[[take]]['Male'] )) %*% matrix(coef(linear.size))
  max.GPA <- c(max.GPA)
  sizes <- seq(min(proctmp$X[,2]) , max.GPA, length.out=1000)
  ...
  #Also use real maximum as the maximum in the sequence below
  min.real <- min(subset(landmark_data,Name==take)$Length[males])
  real.sizes <- seq(log10(min.real) , log10(max.sizes.separate[[take]]['Male']) ,
                    length.out=1000)
  
  preds <- plotAllometry(proctmp2,size=10^sizes, method = 'PredLine',logsz = F)$PredLine
  dev.off()
  
  preds.separate[[taxon]]$PredLine$Males <- data.frame('Predicted_shape'=preds,'CS'=10^sizes,
                                                       'SCL'=10^real.sizes)
  ...
  preds.separate[[taxon]]$CAC$Males <- data.frame('Predicted_shape'=preds,'CS'=10^sizes,
                                                  'SCL'=10^real.sizes)
  ######
...
for ( i in 1:length(taxa)){
  ...
  #PredLine- Female
  plot(preds.separate[[take]]$PredLine$Females[,2:1],xlab='CS',ylab='Predicted shape',
       main = paste0(take,'\n(Females)'))
  
  thr <- 0.85 #% upper quantile of size-shape association
  
  distances <- 1-as.matrix(dist(preds.separate[[take]]$PredLine$Females[,1]))[1000,]
  minimum <- which(distances>thr)[1]
  x=preds.separate[[take]]$PredLine$Females[,2] [minimum ]
  #x
  rect(xleft = x, xright = max(preds.separate[[take]]$PredLine$Females[,2])*1.2,
       ybottom =min(preds.separate[[take]]$PredLine$Females[,1])*1.2,
       ytop =max(preds.separate[[take]]$PredLine$Females[,1])*1.2,
       border=NA, col=adjustcolor('grey',alpha.f = 0.2))
  
  abline(v=x,col='red')
  preds.separate[[take]]$threshold.F <- c()
  preds.separate[[take]]$threshold.F['PredLine'] <- x
  
  #CAC- Female
  distances <- 1-as.matrix(dist(preds.separate[[take]]$CAC$Females[,1]))[1000,]
  minimum <- which(distances>thr)[1]
  x=preds.separate[[take]]$CAC$Females[,2] [minimum ]
  
  abline(v=x,col='red',lty=2)
  
  preds.separate[[take]]$threshold.F['CAC'] <- x
  
  legend('topleft',legend=c('PredLine','CAC'),lty=c(1,2),col='red',bty='n')
  

  #PredLine- MALE
  plot(preds.separate[[take]]$PredLine$Males[,2:1],xlab='CS',ylab='Predicted shape',
       main = paste0(take,'\n(Males)'))
  
  thr <- 0.85 #% of the maximum size shape
  
  distances <- 1-as.matrix(dist(preds.separate[[take]]$PredLine$Males[,1]))[1000,]
  minimum <- which(distances>thr)[1]
  x=preds.separate[[take]]$PredLine$Males[,2] [minimum ]
  #x
  rect(xleft = x, xright = max(preds.separate[[take]]$PredLine$Males[,2])*1.2,
       ybottom =min(preds.separate[[take]]$PredLine$Males[,1])*1.2,
       ytop =max(preds.separate[[take]]$PredLine$Males[,1])*1.2,
       border=NA, col=adjustcolor('grey',alpha.f = 0.2))
  
  abline(v=x,col='red')
  preds.separate[[take]]$threshold.M <- c()
  preds.separate[[take]]$threshold.M['PredLine'] <- x
  
  
  #CAC- MALE
  distances <- 1-as.matrix(dist(preds.separate[[take]]$CAC$Males[,1]))[1000,]
  minimum <- which(distances>thr)[1]
  x=preds.separate[[take]]$CAC$Males[,2] [minimum ]
  
  abline(v=x,col='red',lty=2)
  
  preds.separate[[take]]$threshold.M['CAC'] <- x
  
  legend('topleft',legend=c('PredLine','CAC'),lty=c(1,2),col='red',bty='n')
  
  }

thresholds.separate <- lapply(lapply(preds.separate, function(x) x[c('threshold.F','threshold.M')] ),
       function(x) do.call(rbind,x))

#Thresholds
lapply(taxa , function(x) thresholds.separate[[x]] / sapply(preds.separate[[x]]$PredLine,function(x) max(x[,'CS']))  )



#additional curves plot

separate.thresh <- lapply(max.sizes.separate, function(x) x*0.71)
...
for ( i in both_curves){
  ...
  sapply(separate.thresh[[i]] ,function(x) abline(v=x, col='red')   )
